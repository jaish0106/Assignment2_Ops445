...
"ajjayaratnam". No code in this file is copied from any other source 
...
def parse_command_args() -> object:
    """
    Set up argparse. This function parses command-line arguments and returns them.
    """
    parser = argparse.ArgumentParser(
        description="Memory Visualiser -- See Memory Usage Report with bar charts",
        epilog="Copyright 2023"
    )
    # Add -l/--length argument to specify graph length
    parser.add_argument(
        "-l", "--length",
        type=int,
        default=20,
    ...
    # Add -H/--human-readable argument to enable human-readable output
    parser.add_argument(
        "-H", "--human-readable",
        action="store_true",
        help="Display memory in human-readable format (e.g., MiB)."
    )
    # Add -r/--running-only argument to filter running processes only
    parser.add_argument(
        "-r", "--running-only",
        action="store_true",
        help="Show only running processes."
    )
    # Add positional argument for specifying a program
    parser.add_argument(
        "program",
        type=str,
        nargs='?',
...
def rss_mem_of_pid(pid):
    ...
    # Smaps file is opened to read its content
    with open(smaps_file, 'r') as f:
        data = f.read()

    # Extract the VmRSS value from the file content
